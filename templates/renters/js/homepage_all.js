...
function is_element_disabled(e) {
	return ($(e).attr('data-disabled') == 'true');
}

function disable_element(e) {
	$(e).attr('data-disabled', 'true').addClass('disabled');
}

function enable_element(e) {
	$(e).attr('data-disabled', null).removeClass('disabled');
}

// emulate some of rails3 remote form functionality
function remote_forms(selector_prefix) {
	na.remote_forms(selector_prefix);
}

na.remote_forms = function(selector_prefix, callbacks) {
	callbacks = callbacks || {};
	...
	$(selector_prefix + 'form[data-remote=true]').livequery(function(){
		...
		var options = {
			success: function(response) {		
				if(typeof(callbacks.after_ajax_success) == "function") {
					callbacks.after_ajax_success(elm);
				}
			}
    };
    elm.ajaxForm(options);
		...
		elm.bind('ajaxSend', function(e, request, options) {
			if(typeof(callbacks.after_ajax_send) == "function") {
				callbacks.after_ajax_send(elm);
			}
		});
		
		elm.bind('ajaxComplete', function(e, request, options) {
			if(typeof(callbacks.after_ajax_complete) == "function") {
				callbacks.after_ajax_complete(elm);
			}
		});
...
na.submit_button = function(button_elm, callbacks, form_elm) {
	callbacks = callbacks || {};
  form_elm = form_elm || null;
	var e = $(button_elm);
	var self = this;
	
	self.handler = function() {
		// check to make sure button is not disabled.
		if(is_element_disabled(this)) {
			 return false;
		}
    ...
    if(typeof(form) == 'object') {
      ...
      // before_submit
      if(typeof(callbacks.before_submit) == 'function') {
        allow_submit = callbacks.before_submit(e, this);
      ...
      if(allow_submit) {
        form.submit();
        // after_submit
        if(typeof(callbacks.after_submit) == 'function') {
          callbacks.after_submit(e);
        }
      }
    }
    return false;
	...
	self.live = function() {
		e.unbind('click').die('click').live('click', self.handler);
	};
	
	self.init = function() {
		e.unbind('click').die('click').click(self.handler);
	};
	
	return self;
};

function submit_button(button_elm, callbacks, form_elm) {
	new na.submit_button(button_elm, callbacks, form_elm).init();
}

na.scrollTop = function() {
	$('body,html').animate({
		scrollTop: 0
	}, 400);
};
...
na.wysiwyg_editor = {};
na.wysiwyg_editor.basic = function(selector) {
	try {
		$(selector).ckeditor(function() {}, {toolbar: 'Basic'});
	} catch(err) {}
};
...
/*
 * String.append_url_params
 * append params to url
 */
if(String.prototype.append_url_params == null) {
  String.prototype.append_url_params = function(params) {
	  _url = this;
    _url += (_url.split('?')[1] ? '&':'?') + params;
    return _url;
  };
}
...
$(document).ready(function() {
	...
	na.remote_forms('#cboxLoadedContent ', {
		after_ajax_success: function() {
			$.colorbox.resize();
			na.init_global_events();
		},
		after_ajax_send: function(elm) {
			elm.hide();
...
function fb_oauth_param(token) {
	return 'oauth_token=' + escape(token);
}

function fb_login(return_to) {
	$('.btnFB').replaceWith(ajax_loader_centered());
	
  if(!return_to)
    return_to = document.location.href;
    
	var url = '/user_sessions/create?return_to=' + escape(return_to);

	FB.getLoginStatus(function(response) {
	  if (response.authResponse) {
	    // logged in and connected user, someone you know
			document.location.href = url + '&' + fb_oauth_param(response.authResponse.accessToken);
	  } else {
...
na.facebook = {signup:{}};
na.facebook.signup.complete_form = function() {
	var custom_fb_status = $('#fbPostMessage');
	$('#completeFB input[name=fb_post_status]').click(function() {
		if($(this).val() == '1') {
			custom_fb_status.fadeIn();
		} else {
			custom_fb_status.hide();
		}
	});
};
...
$(document).ready(function(){
	na.facebook.signup.complete_form();
});



na.share_buttons_init = function() {
	$('.shareButton.modal, .customShareButton.modal').colorbox({
		href: function() {
			return $(this).attr('data-url');
		}
	});
	
	// ensure the pinterst button's iframe doesn't create extra unwanted white space
	$('.pinterest iframe').css('width', '44px');
};

$(document).ready(function() {
	na.share_buttons_init();
});
...
na.listing = {};
na.listing.id = function() {
	return $('#listingContent').attr('data-id');
};
...
var ViewListing = function() {
	var image_gallery = function(selector) {
		$(selector).galleria({
				width: 590,
				height: 420,
				debug: false,
        data_selector: 'img',
				dummy: '/images/default_image/listing/normal/missing.jpg',
        ...
        showCounter: false,
        transitionSpeed: 200,
				maxScaleRatio: 1 // disable auto scaling of images
    });
	};
	
  if($('#listingContent').length > 0){
    // Initialize Galleria for Photos
 		image_gallery('#images .galleria');
	} else {
		return false;
  }
  ...
  $('#listingDetailWidget').tabs({
    select: function(event, ui) {
      ...
      var p_id = p.attr('id');
			...
			// load galleria for floor plans on initial tab click
			var selector = '#' + p_id + ' .galleria';
			if(p_id == 'floorPlans' && $(selector + ' .galleria-container').length == 0) {
				image_gallery(selector);
			}
      ...
      if(p_id == 'listingMapWrapper') {
				if($('#listingMap div').length == 0) {
					p.children('div').html('loading...');
	        window.setTimeout(function() {
						na.maps.listing.single();
	        }, 100);
				}
      }
    }
  ...
  $('#listingContent #info').tabs();
	...
	$('.nestio-add-btn').live('click', function() {
		ga_track_event('Listing Details', 'Click', 'nestio button');
	});

	na.listing.flag.init();
	
	// hide reviews section in broker card if broker doesn't have reviews
	var bc_reviews = $('#listingContent .brokerCard .reviews');
	if(bc_reviews.find('li').length == 0) {
		bc_reviews.hide();
	}
};
...
na.search = {};
na.search.fancy_dropdowns_init = function() {
	// Nav
...
var HowItWorksTour = {
  setup: function() {
    var self = this;
    if($('ul.ajaxContent').length == 0) {
      return false;
    }
    // sub navigation logic
    $('ul.ajaxContent li').click(function() {
      var step = $(this).attr('data-step');
      
      $('ul.ajaxContent li').each(function() {
...
na.feedback_form_init = function() {
	$(".feedbackBox").colorbox({
    onComplete: function() {
...
var ContactAgent = function() {
  ...
  $(".contactAgentBtn").colorbox({
    onComplete: function() {
      ...
      var params = '?listing_id=' + listing_id + '&broker_id=' + broker_id + '&location=' + $(this).attr('data-location') + '&cb=' + cache_buster();
...
na.bulkEditActionButtons = function() {
	$('.bulkEditActions .button').click(function() {
		$(this).parents().find('#bulk_action').val($(this).attr('data-action'));
	});
};
...
function neighborhood_options_for_fancy_select(borough_id, after_html) {
  ...
  Ajax.get_neighborhoods_by_borough_id(borough_id, function(json) {
    ...
    $.each(json.neighborhoods, function(i, n) {
      html += '<li>';
      html += '<input type="checkbox" value="' + n.id + '" /> ' + n.name;
      html += '</li>';
      r++;
      if(r >= per_col) {
...
na.init_global_events = function() {
	$('.infoModal').colorbox({inline:true});
	$('.ajaxModal').colorbox({
		width: function() {
  ...
  $('.dateformat-m-sl-d-sl-Y').each(function() {
    ...
    $(this).datepicker({ dateFormat: 'mm/dd/yy', showAnim: 'fadeIn', minDate: $(this).attr('data-min-date') });
  });

  rollover_buttons();
	na.selectAllRowsCheckbox();
	na.bulkEditActionButtons();
  ...
  */
  ...
  $('.tooltip:not(.clickable):not(.infoModal):not(#flagListing)').click(function() {
    return false;
  });
  ...
  $('.alert .alertOpen').click(function() {
	...
	// reveal more content after button click
	$('.revealMore').live('click', function() {
...
$(document).ready(function() {
  ...
  */
  ...
  new na.submit_button('.submitButton').live();
  ...
  $('.moreContent').livequery('click', function () {
  ...
  $('#landing #listingSERPTable .thumbnail img').click(function() {
	...
	// temp home for user surveys
	$('.userSurvey a').click(function() {
		var p = $(this).parent();
		
		ga_track_event(p.attr('data-type'), 'Survey', $(this).text());
		
		p.html('<strong>Thank you for your valuable and much-appreciated feedback!</strong>');
		return false;
	});
...
na.maps = {
	...
	listing: { 	
		...
		serp: function() {
			var self = this;
			// disable map feature on serp for mobile users
			if($.browser.mobile) {
				return false;
			}
...
function initialize_maps() {
	if($('#listingsMap').length > 0) {
		na.maps.listing.serp();
	}
}


$(document).ready(function() {
	// don't bother to load JS file for map on SERP if mobile browser
	if($('#listingMap').length > 0 || ($('#listingsMap').length > 0 && !$.browser.mobile)) {
		na.maps.loadScript();
	}
});
...
$(document).ready(function() {
  // home page sign up
  rollover_button('#btn_signup');
  
  version_b = window.location.toString().search(/\/vb/) > -1


  // mouseover listing thumbnail
  $('#listings li').mouseover(function() {
    if($(this).hasClass('last') && !version_b) return false;
    $(this).children('.detailsOverlay').show();
  });
  // mouseout listing thumbnail
  $('#listings li').mouseout(function() {
    if($(this).hasClass('last') && !version_b) return false;
    $(this).children('.detailsOverlay').hide();
  });
  
  // click listing thumbnail
  $('#listings li, #listings li .detailsOverlay').click(function(){
    var elm = ($(this)[0].nodeName.toLowerCase() == "li") ? $(this) : $(this).parent();
    var id = elm.attr('id');
    ga_track_event('Homepage', 'Click', 'listings grid: listing');
    // make it seem like you clicks on <a> when overlay is shown
    if($(this)[0].nodeName.toLowerCase() != "li") {
      window.location = $(this).siblings('a').attr('href');
    }
  });

  $('#listings .search').click(function() {
    ga_track_event('Homepage', 'Click', 'listings grid: search');
  });

  $('#masthead .brokers').click(function() {
    ga_track_event('Homepage', 'Click', 'broker signup');
  });
  
  $('#feature a').click(function() {
    ga_track_event('Homepage', 'Click', 'new feature');
  });

  $('#btn_signup_a').click(function() {
    ga_track_event('Homepage', 'Click', 'create free profile (orange button)');
  });
  ...
  $('#benefits li.link a').click(function() {
    ga_track_event('Homepage', 'Click', 'benefits: ' + $(this).text());
  });
  $('#social li a').click(function() {
    ga_track_event('Homepage', 'Click', 'social: ' + $(this).parent().attr('class').replace(/active/, ''));
  });
  $('#reviews a').click(function() {
    ga_track_event('Homepage', 'Click', 'broker reviews');
  });
